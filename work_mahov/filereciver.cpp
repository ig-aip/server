...
void FileReciver::resolveHelloWord() {
    auto self = shared_from_this();
    std::shared_ptr<HelloWord> word = std::make_shared<HelloWord>();
    asio::async_read(socket_,
        boost::asio::buffer(&*word, sizeof(*word)),
        [self, word](boost::system::error_code er, std::size_t bytes) {
            if (!er) {
                std::cout << "ip_v4: " << self->socket_.remote_endpoint().address().to_v4() << " port: " << self->socket_.remote_endpoint().port() << std::endl;
                if (word->name == "send") {

                }
                else if (word->name == "save") {
                    self->set_settings_read();
                }
                else if (word->name == "open") {
...
void FileReciver::set_settings_read(){
    auto self = shared_from_this();
    std::shared_ptr<tcpHeader> header = std::make_shared<tcpHeader>();
    asio::async_read(socket_,
        boost::asio::buffer(&*header, sizeof(*header)),
        [this, self, header](boost::system::error_code er, std::size_t bytes){
            if(!er){
